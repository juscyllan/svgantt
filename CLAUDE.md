...
Zero-dependency SVG Gantt chart library. Same codebase renders in browsers (Astro, vanilla) and Obsidian (Electron).
...
```typescript
import { createGantt } from 'obsidian-gantt';
import 'obsidian-gantt/styles';
...
- `jusc.dev` — Astro site (replaces inline GanttChart.astro)
- Future: Obsidian plugin (reads vault frontmatter → renders Gantt)
