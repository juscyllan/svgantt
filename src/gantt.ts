		...
		if (opts.tooltip) {
			for (const item of barRects) {
				...
				item.el.addEventListener("mouseenter", () => {
					...
					tooltipEl.querySelector<HTMLElement>(
						".og-tooltip-name",
					)!.textContent = item.task.name;
					const startD = parseDate(item.task.start);
					const endD = parseDate(item.task.end);
					tooltipEl.querySelector<HTMLElement>(
						".og-tooltip-dates",
					)!.textContent =
						`${formatDate(startD, opts.locale)} \u2192 ${formatDate(endD, opts.locale)}`;
					const dur = daysBetween(startD, endD) + 1;
					tooltipEl.querySelector<HTMLElement>(".og-tooltip-dur")!.textContent =
						`${dur} dias`;

					const dot = tooltipEl.querySelector<HTMLElement>(".og-tooltip-dot")!;
					if (sc.dotBorder) {
						dot.style.background = "transparent";
						dot.style.border = `1.5px solid ${sc.dotBorder}`;
					} else {
						dot.style.background = sc.fill;
